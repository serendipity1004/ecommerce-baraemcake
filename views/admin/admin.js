...
jQuery(document).ready(function ($) {
    ...
    var datatable = $('#order-datatable').mDatatable(datatableOptions);

    $('#order-datatable').on('click', 'tr.m-datatable__row', function (e) {
        if ($(this).parent().is('thead')) {} else {
            var id = $(this).find('td[data-field="_id"]').text();

            var detailsTable = $(this).closest('tr').next('tr').find('.m-datatable__detail');

            $.post('/api/admin/order/details', { id: id }, function (returnedData) {
                $(detailsTable).empty();
                var template = createDetailRowDom(returnedData.result);
    ...
    $('#delivery-done').click(function (e) {
        ...
        $('#order-datatable input:checkbox').each(function (index, item) {
            var template = {};
            if ($(item).is(':checked')) {
                var id = $(item).parents('td').siblings('td[data-field="_id"]').text();
